...
void log()
{
	...
	while (fin)
	{
		content[sum][length % (SEND_MAX - SEND_TOP)] = t;
		length++;
		if (length % (SEND_MAX - SEND_TOP) == 0)
		{
			sum++;
			length = 0;
		}
...
void want_to_sendfile(int want_to_send)
{
	...
	else
	{
		...
		for (int j = 0; j < 4; j++)
			sendBuf[20 + j] = (char)((SEND_MAX - SEND_TOP) >> (24 - 8 * j));
	}
...
//在窗口内，如果大于等于窗口最小的那个，窗口移动到ack+1
//如果窗口cwnd<ssthresh，则cwnd*=2
//如果cwnd>=ssthresh，则cwnd++
//使用lastack记录上一个ack，同时更新lastack=ack
//使用lastacksum记录上一个ack的个数，这里更新lastacksum=1
//如果不在窗口内，判断是否ack是否为lastack，是的话lastacksum++
	...
	while (1)
	{
			...
			&& verifychecksum())
		{
			...
			else
			{
				if (lastacksum >= 3)
				{
					//cout << "ack>3快速重传" << "  cwnd" << cwnd << endl;
					ssthresh = cwnd / 2;
					cwnd = ssthresh + 3;
					base = has_send = ack + 1;
					want_to_sendfile(ack+1);
				...
				else 
				{
					...
					{
						lastacksum++;
						//cout << "收到重复的ack了" << "  ack" << ack << " lastacksum" << lastacksum << endl;
					}
				}
			}
			cout<<"窗口大小 "<<has_send-base << "  左 " << base << "  右 " << has_send << endl;
			ReleaseMutex(hMutex);
		}//释放互斥量锁
...
int main()
{
	...
	while (1)
	{
		...
		while (1)
		{
			if (((has_send - base) < cwnd) && has_send < seq + sum + 1)
			{
	...
	cout << "发送" << (sum * (SEND_MAX - SEND_TOP) + endfilesize) << "字节" << (t_end - t_start) << "毫秒" << endl;
	cout << "平均吞吐率" << (sum * (SEND_MAX - SEND_TOP) + endfilesize) * 8 * 1.0 / (t_end - t_start) * CLOCKS_PER_SEC << " bps" << endl;
	closesocket(sockSrv);
	WSACleanup(); return 0;
	ReleaseMutex(hMutex);
}
